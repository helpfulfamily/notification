...
import family.helpful.notification.message.model.DialogContent;
...
public class DialogListener {
    ...
    public void publishProblemContent(Message<DialogContent> message) {

        logger.info(" received PUBLISH_DIALOG_CONTENT [" + message.toString() + "] ");


        Message resultMessage = MessageBuilder.withPayload(message.getPayload())
                .setHeader("PUBLISH_DIALOG_CONTENT"
                        , EnumActionStatus.SUCCESS.name()).build();

        this.template.convertAndSend("/topic/pushNotificationDialogContent", resultMessage);
